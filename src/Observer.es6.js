...
export class Observer {
    ...
    error(errorValue){
        ...
        if(this.closed || !(myerror instanceof Function)) return;
...
export class Observable {
    ...
    subscribe(observerOrNext, onError, onComplete) {
        if(!observerOrNext) return _console().error("Observer subscription attempted, but the arguments are empty", this, observerOrNext);

        if(observerOrNext instanceof Function){
            observerOrNext = new Observer(observerOrNext, onError, onComplete);
        }
        if(!(observerOrNext instanceof Observer)) return _console().error("Observer subscription attempted, but the argument was not an Observer.", this, observerOrNext);
    ...
    static from(observable){
        ...
        }else if( typeof observable[Symbol.iterator] != "undefined" ){
